...
fn main() {
    ...
    let (ing, cache_attr) = match cache::get(&root, cfg.no_cache) {
        cache::Source::Hit { ing } => (ing, "hit"),
        cache::Source::Rebuilt { ing } => (ing, "rebuilt"),
    };
    ...
    let doc = match cfg.verb() {
        cli::Verb::Map => {
            ...
            serialize::serialize(&ing, &g, &run, &root, cache_attr)
        }
